...
func init() {
	...
	logrus.SetFormatter(InstanceOfFairFaxFormatter())
}

func preLog() *Entry {
	pc, file_name, line_num, _ := runtime.Caller(2)
	func_name := runtime.FuncForPC(pc).Name()
	entry := logrus.WithFields(logrus.Fields{
		"_loc":  fmt.Sprintf("%s:%d", path.Base(file_name), line_num),
		"_func": func_name,
	})
...
func preLogWithFields(fields *Fields) *Entry {
	_new_entry := logrus.WithFields(logrus.Fields(*fields))
	return &Entry{LogrusEntry: _new_entry, Pre: nil}
}
